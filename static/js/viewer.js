(function () {
  ...
  async function init(opts) {
    ...
    function getHailValueFromFeature(feature) {
      ...
      const possibleKeys = [
        'Hail_Size', 'hail_size', 'HailSize', 'hailsize',
        'max_hail_diameter', 'Max_Hail_Diameter', 'MAX_HAIL_DIAMETER',
    ...
    function getPointRadius(hailValue) {
      ...
      const maxRadius = 12;
      const range = (hailMax - hailMin) || 1;
      const normalized = Math.max(0, Math.min(1, (hailValue - hailMin) / range));
      const calculatedRadius = baseRadius + normalized * (maxRadius - baseRadius);
      return calculatedRadius * (pointRadiusMultiplier / 6);
    }
    ...
    const CanvasContinuousLayer = L.Layer.extend({
      ...
      _update: function () {
        ...
        if (!this._canvas || !this._canvas.parentNode) return;
        ...
        try {
          ...
          console.warn('[Viewer] Canvas position update deferred:', e.message);
          return;
        }
    ...
    function getFeatureStyle(feature) {
      ...
      if (displayMode === 'continuous') {
        return { fillColor: 'transparent', fill: false, stroke: false, weight: 0, opacity: 0, fillOpacity: 0 };
      } else {
    ...
    function updatePointsStyle() {
      ...
      pointsLayer.eachLayer(layer => {
        if (layer.setStyle && layer.feature) {
          const hailValue = getHailValueFromFeature(layer.feature);
          layer.setStyle({
            radius: getPointRadius(hailValue),
    ...
    opacitySlider?.addEventListener('input', function () {
      currentOpacity = safeNum(this.value, 0.7);
      if (opacityValue) opacityValue.textContent = currentOpacity.toFixed(1);
      refreshLayerStyles();
    });
          ...
          if (data.features?.length) {
            pointsLayer = L.geoJSON(data, {
              pointToLayer: (feature, latlng) => {
                const hailValue = getHailValueFromFeature(feature);
    ...
    function getCurrentSettings() {
      return {
        ...
        pointRadius: safeNum(document.getElementById('pointRadius')?.value, pointRadiusMultiplier),
